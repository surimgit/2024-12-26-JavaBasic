...
//	[접근제어자] 반환타입 메서드명(매개변수타입, 매개변수명, ...) { 메서드의 기능 구현}
	void method1() {
...
//	클래스 메서드에서는 인스턴스 변수 사용 및 인스턴스 메서드 호출이 불가능함.
//	인스턴스 변수 및 인스턴스 메서드를 사용하지 않는 기능에 주로 사용됨
	static int classMethod(int arg1) {
...
//		void: 타입없음
	...
	static void printBMI(double x, double y) {
		x = (183*183);
		y = 84 / x;
		System.out.println("bmi: " + y);
	}
//	함수의 목적
//	1. 기능을 미리 정의해두고 사용할 때는 호출하여 사용할 수 있도록 함
...
//		인스턴스.속성명;
		...
		instance1.method1();
//		클래스 변수를 사용할 때 주의할 점
//		클래스 변수는 모든 인스턴스에 영향을 미치기 때문에 정적(클래스를 통해서)접근하는걸 권장함.
//		일반적으로 클래스 변수는 final 키워드와 함께 사용함
		...
		galaxy24.os= "Android";
		galaxy24.telNumber = "010-9999-9999";
...
//		System.out.println("bmi: " + y);
		...
		result = TriangleMath.getCircumference(3, 4, 5);
	}
}