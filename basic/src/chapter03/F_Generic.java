...
// 제너릭: 클래스나 메소드에서 사용할 데이터 타입을 외부에서 지정하는 방법

// Test1, Test2, Test3의 문제점
// 1. 코드 중복 발생: 유지 보수성, 개발 속도 저하
class Test1{
	String subject;
	int score;
}
...
//		int score = (int) test.get(0).score;
		
//		제너릭으로 지정하면 컴파일시 타입을 결정해 원하는 데이터 타입으로 사용 가능
//		제너릭의 타입을 전달할 때 반드시 참조형 데이터 타입만 가능
		Test5<Integer> instance1 = new Test5<>();
		instance1.subject = "영어";
//		instance1.score = "우수";
		instance1.score = 100;
...
//		MyClass 혹은 MyClass의 상위 클래스만 전달 가능
		superMethod(superList);
		superMethod(myList);
//		superMethod(subList);
		
//		메소드의 매개타입은 직접 지정하지 않고 매개변수의 타입을 따라감
